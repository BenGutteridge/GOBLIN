...
def solve_ls_with_bias_cv(x: Tensor, y: Tensor, lambdas: List[float] = None,
                          k_folds: int = 10) -> Tuple[Tensor, Tensor]:
    if lambdas is None:
        lambdas = [1e-15, 1e-8, 1e-6, 1e-4, 1e-2]
...
class LeastSquaresTransform(BaseTransform):
    def __init__(self, ls_num_layers: int, dataset_name: str, num_feat: int,
                 seed: int, cache_root: str):
        super().__init__()
        self.ls_num_layers = ls_num_layers
        self.mean_aggr = SimpleConv(aggr='mean')
        self.cache_path = os.path.join(cache_root, dataset_name,
                                       f'{num_feat}feat', f'seed{seed}')
    ...
    def forward(self, data):
        ...
        self._save_cache(xy_conversions,
                         new_key_list=list(set(xy_conversions.keys()) - set(xy_keys)))
