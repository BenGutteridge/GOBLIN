...
class ConvGAT3dAggr(torch.autograd.Function):
    @staticmethod
    def forward(ctx, h_in, adj_rowptr, adj_indices, att_src, att_dst,
                out_node_count, work_group_size=None):
        torch.cuda.set_device(h_in.device)
        num_features_per_node = h_in.shape[1]
        num_channels = att_src.shape[1]
        ...
        gat_forward[(num_nodes, num_work_groups)](
            ...
            num_features_per_node, num_channels, work_group_size, num_warps=32)
    ...
    def backward(ctx, dh_out):
        ...
        gat_backward[(num_nodes, num_work_groups)](
            dh_src, adj_rowptr, adj_indices, h_in, dh_out, att_src, att_dst,
            datt_src_out, datt_dst_out,
            num_features_per_node, num_channels, work_group_size, num_warps=32)
...
def gat_forward(
        ...
        IN_CHAN: tl.constexpr, ATT_CHAN: tl.constexpr, WG_SIZE: tl.constexpr
):
    ...
    att_offsets = (tl.arange(0, WG_SIZE) + tl.program_id(1) * WG_SIZE) % ATT_CHAN
    att_valid_mask = att_offsets < ATT_CHAN
    ...
    att_src_vec = tl.load(att_src + att_offsets, att_valid_mask, feat_zeros)
    att_dst_vec = tl.load(att_dst + att_offsets, att_valid_mask, feat_zeros)
    ...
    for index in range(col_count):
        ...
        max_attn = tl.maximum(max_attn, attn_ij)

    for index in range(col_count):
        node_index_j = tl.load(adj_indices + col_start + index)
        neighbor_feat_j = tl.load(h_src + node_index_j * IN_CHAN + feat_offsets, feat_valid_mask, feat_zeros)
        attn_ij = tl.sum(att_dst_vec * feat_i + att_src_vec * neighbor_feat_j, axis=0)
        attn_ij = tl.where(attn_ij < 0, 0.2 * attn_ij, attn_ij)
        attn_ij = tl.exp(attn_ij - max_attn)
        aggr_sum += attn_ij * neighbor_feat_j
        attn_sum += attn_ij

    attn_sum = tl.where(attn_sum == 0, 1, attn_sum)
    aggr_mean = aggr_sum / attn_sum
...
def gat_backward(
    ...
    for index in range(col_count):
        ...
        neighbor_feat_j = tl.load(h_in + node_index_j * IN_CHAN + feat_offsets, feat_valid_mask, feat_zeros)
        attn_ij = tl.sum(att_dst_vec * feat_i + att_src_vec * neighbor_feat_j, axis=0)
        attn_ij_leaky = tl.where(attn_ij < 0, 0.2, 1.0)
    ...
    tl.store(datt_src_out + att_offsets, datt_src, att_valid_mask)
    tl.store(datt_dst_out + att_offsets, datt_dst, att_valid_mask)
